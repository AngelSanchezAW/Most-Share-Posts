...
Este archivo contiene el código para el widget. Se requiere tanto por el lado de administración y la parte delantera.
...
class Most_Shared_Posts extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		// Esta parte muestra el titulo del Widget, he modifoca un poco para agregar una imagen despues del texto. 	
			echo $before_title. $title.'<img align="right" style="margin-top:-12px;" src="http://www.azulweb.net/wp-content/uploads/2015/10/msp-tendencia.png">'. $after_title;
		...
		while ( $posts_in_range->have_posts() ) : $posts_in_range->the_post();
           ...
           //Sacar imagen destacada
			...
			if (!$suppress_icons)
			{
				...
				echo '<div class="msp-tendenciaimg"><img src="http://www.azulweb.net/wp-content/uploads/2015/10/msp-tendencia.png"></div>';
				echo " &nbsp; ";
				}
				...
				$total_tendencia = $plusones + $tweets + $fb_likes;
                
                echo "|";
                echo " &nbsp; ";

				echo $total_tendencia.' PERSONAS HABLANDO DE ESTO';
				
				echo '</div>';
				echo '<a href="' . get_permalink() .'?utm_source=interno&utm_medium=widgets&utm_campaign=mps'. '" rel="bookmark">' .'<p class="msp-title">'. get_the_title() .'</p>'.'</a>';
